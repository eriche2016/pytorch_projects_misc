...
class Field(object):
    ...
    def preprocess(self, x):
        ...
        if self.sequential and isinstance(x, str):
            x = self.tokenize(x)
        if self.lower:
            x = Pipeline(six.text_type.lower)(six.text_type(x)) # such that we can convert x(string) to unicode
    ...
    def numericalize(self, arr, device=None, train=True):
        ...
        If the field has include_lengths=True, a tensor of lengths will be included
        in the return value 
...
class BucketIterator(Iterator):
    ...
    def init_epoch(self):
        if self.sort:
            ...
            self.batches = pool(self.data(), self.batch_size, self.sort_key)
        if not self.repeat: 
            self.iterations = 0 
