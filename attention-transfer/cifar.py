...
import math
import cv2
from tqdm import tqdm # make loops show a smart progress meter, just wrap any iterable with tqdm(iterable)
import pandas as pd #  
import torch
import torch.optim
import torch.utils.data # an abstract class representing a Dataset  
from torchvision import cvtransforms 
import torchvision.datasets as datasets #  contains popular datasets: COCO(captioning and detection), LSUN classification, ImageFolder, ImageNet-12, CIFAR10 and CIFAR100
...
def create_dataset(opt, mode):
    ...
    return ds.transform({0: train_transform if mode else convert}) # mode is True, use train_transform, mode is False, using convert
